...
Runge-Kutta is a more accurate method, as we will see. It involves taking multiple evaluations across the time step of length h. The improved Euler method is a simple example of this. Instead of using $f(u_i,t_i)$ as the derivative, we use $f(u_i+(h/2)f(u_i,t_i), t_i + h/2)$. The derivation of IE2 is the same, approximate the derivative using the same forward difference, and then solve for $u_{i+1}$. The pointwise error is O(h^3) and the truncation error becomes O(h^2), as shown in the image. RK4 is another method, taking 4 evaluations across the time step. The first evaluation is $k1= hf(u_i,t_i)$, the second is $k_2=hf(u_i +(1/2)k_1, t_i +(1/2)h)$, the third is $k_3=hf(u_i +(1/2)k_2, t_i +(1/2)h)$, the fourth is $k_4= h(u_i+k_3, t_i + h)$. Then $u_{i+1}= (1/6)k_1 + (1/3)k_2 +(1/3)k_3 + (1/6)k_4$. 
